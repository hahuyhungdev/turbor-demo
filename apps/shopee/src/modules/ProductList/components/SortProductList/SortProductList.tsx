...
import { sortBy, order as orderConstant } from '@/configs/product'
...
export default function SortProductList({ queryConfig, pageSize }: Props) {
  ...
  const handleSort = (sortByValue: Exclude<ProductListConfig['sort_by'], undefined>) => {
    const newQueryConfig = omit({ ...queryConfig, sort_by: sortByValue }, ['order'])
    setSearchParams(newQueryConfig)
  }

  const handlePriceOrder = (orderValue: Exclude<ProductListConfig['order'], undefined>) => {
    const newQueryConfig = { ...queryConfig, sort_by: sortBy.price, order: orderValue }
    setSearchParams(newQueryConfig)
  }
            ...
            {page === 1 ? (
              ...
              <Link
                to={{
                  ...
                  search: createSearchParams({
                    ...queryConfig,
                    page: ((page as number) - 1).toString() || '1'
                  }).toString()
                }}
            ...
            {page === pageSize ? (
              ...
              <Link
                to={{
                  ...
                  search: createSearchParams({
                    ...queryConfig,
                    page: ((page as number) + 1).toString()
                  }).toString()
                }}
                className='flex h-8 w-9  items-center justify-center rounded-bl-sm rounded-tl-sm bg-white  shadow hover:bg-slate-100'
              >
